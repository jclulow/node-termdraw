...
_redo()
{
	...
	if (height === 0) {
		...
		/*
		 * We clear any offset here to avoid unnatural jumping around
		 * later on when we have enough lines that we would use it.
		 *
		 * This can happen if a consumer calls .offset() or .moveto()
		 * before adding a screen's height worth of lines, or if the
		 * height of the screen is resized past the number of lines.
		 */
		self.lb_roffset = null;
	} else if (nlines > height) {
